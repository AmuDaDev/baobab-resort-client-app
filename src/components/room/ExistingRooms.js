					...
					<section className="mt-5 mb-5 container">
						...
						<Row>
							...
							<Col md={6} className="d-flex justify-content-end">
								<Link to={"/AddRoom"}><FaPlus /> Add Room</Link>
							</Col>
						</Row>
							...
							<tbody>
								{currentRooms.map((room) => (
									<tr key={room.id} className="text-center">
										...
										<td className="gap-2">
											<Link to={`/EditRoom/${room.id}`} className="gap-2">
												<span className="btn btn-info btn-sm"><FaEye /></span>
												<span className="btn btn-warning btn-sm ml-5"><FaEdit /></span>
											</Link>
											<button	className="btn btn-danger btn-sm ml-5" onClick={() => handleDelete(room.id)}><FaTrashAlt /></button>
										</td>
									</tr>
								))}
							</tbody>
